...
#define vec_cmp_result(a) vmvnq_s32(vceqzq_s32(((vector)a).m))
...
#define vec_blend(mask, a, b) \
  ((vector)vec_or(vec_and(mask.m, ((vector)b).m), \
   vcvtq_s32_f32(((vector)vec_and(vec_not(mask.m), ((vector)a).m)).v))).v
#define vec_fma(a, b, c) vec_add(vec_mul(a, b), c)
#define vec_fnma(a, b, c) vec_sub(c, vec_mul(a, b))
